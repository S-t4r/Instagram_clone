...
- Users have a Profile that shows their profile picture, their first and last name, and their bio. Each user has the ability to edit their information when visiting their profile. The User also has the ability to post a picture with caption. The posts can be liked and commented on by others. The comments can also be liked by other users. Once a post is liked or commented on, or a comment has been liked, a notification will be sent to the user informing them of the action. A post can be edited, and just like comments it can be removed by their owners.
- Once logged in, a user can search for other users by their username, first name, and last name. Users can follow anyone they wish and the posts of their followings will be shown in their Home feed. They also can send direct messages to each other. Paginator is used for loading posts and messages to prevent loading them all at once. To prevent sending unnecessary requests for information that the user already has, the information is cached and will only send new data once there has been a modification on the data, otherwise data value will be set to cached values.
- From every aspect this project is built as a Single Page Application. It was a real challenge but I noticed something along the way, everything's possible it just needs time to be spent on.
...
> **Note**: The UI has been recently updated with modern design patterns, Google-style forms, and improved user experience.
...
3. (Optional) Set up the React frontend:
   ```bash
   cd frontend
   npm install
   npm run build
...
- **Modern Design**: Clean, Instagram-inspired interface with gradient backgrounds and smooth animations
- **Google-style Forms**: Login and registration forms feature a minimal, professional design inspired by Google's Material Design
- **Responsive Layout**: Glassmorphism effects on header and footer with a centered, card-based layout
- **Smooth Interactions**: Hover effects, transitions, and animations throughout the application
- **Card-based Posts**: Modern post cards with shadows, rounded corners, and improved spacing

## Contributing

- This project has been significantly improved with modern UI/UX design patterns and better user experience.
